...
ベクトル `v` とスカラー `c` とに演算子 `*` を適用 (apply) した
式 `v * c` は，各要素をスカラーで乗じたベクトルを与える．

スカラー `c` とベクトル `v` とに演算子 `*` を適用した
式 `c * v` は，各要素をスカラーで乗じたベクトルを与える．
...
ベクトル `v` とスカラー `c` とに演算子 `/` を適用した
式 `v / c` は，各要素をスカラーで除したベクトルを与える．
...
ベクトル `v` とスカラー `c` とに演算子 `.+` を適用した式 `v .+ c` は，
...
`v` の各要素にスカラーを乗じた結果に相当する範囲を与える．
スカラーと範囲とに演算子 `*` を適用した式 `c * v` も，同じ結果を与える．
...
`plot` 関数に対して，同じ寸法の二つのベクトルまたは範囲 `xs`，`ys` を渡すと，
 `xs` , `ys` から一つづつ要素を取り出し，
